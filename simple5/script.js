...
async function fetchRSS(url) {
  try {
    ...
    const items = Array.from(entries).map(item => ({
      ...
      thumbnail: extractImage(item.querySelector('content')?.textContent || item.querySelector('description')?.textContent),
...
function renderFeed(items) {
  ...
  items.forEach(item => {
    const isVideo = item.thumbnail && (item.thumbnail.endsWith('.mp4') || item.thumbnail.endsWith('.webm'));
    const isGif = item.thumbnail && item.thumbnail.endsWith('.gif');
    const feedItem = document.createElement('div');
    feedItem.className = 'feed-item';
    const media = item.thumbnail
      ? (isVideo
          ? `<video class="thumbnail" src="${item.thumbnail}" autoplay muted loop playsinline></video>`
...
function toggleView(view) {
  ...
  // Re-render to ensure titles are correctly positioned in grid view
